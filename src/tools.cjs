    ...
    return new Promise((resolve, reject) => {
      ...
      const request = lib.get(target, { headers: { 'User-Agent': 'StanisTerminal-ToolInstaller', Accept: 'application/octet-stream' }, timeout: 30000 }, response => {
        if ([301, 302, 303, 307, 308].includes(response.statusCode)) {
          ...
          // github.com/.../releases/download/... sempre redireciona para o CDN oficial de releases
          // (*.githubusercontent.com, com token assinado e expiração curta) — único domínio de
          // destino aceito fora do próprio host original; qualquer outro é recusado.
          const next = new URL(response.headers.location, target);
          const allowedCrossDomain = target.hostname === 'github.com' && next.hostname.endsWith('.githubusercontent.com');
          if (next.hostname !== target.hostname && !allowedCrossDomain) return reject(new Error('Redirecionamento para outro domínio recusado.'));
