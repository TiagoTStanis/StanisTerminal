...
Para abrir rapidamente, use a distribuição em pasta: extraia **StanisTerminal-1.8.1-win-x64.zip** uma vez para uma pasta local e abra **Stanis Terminal.exe** dentro dela. Mantenha os arquivos juntos; crie um atalho para esse executável. O comando `pnpm build:zip` gera esse pacote em `dist`.
...
**Novo na 1.8.1 (correção):** a resolução da tela do RDP ficava errada/desproporcional. Causa: o app calculava a resolução a pedir ao servidor lendo o tamanho do painel *antes* dele ter sido dimensionado de verdade pela interface — pegava `0` ou um tamanho de sessão anterior, caía num valor padrão (1280×800) e nunca mais corrigia depois, só escalava por CSS. Agora o painel é dimensionado primeiro; a resolução pedida ao servidor bate com o tamanho real da janela.
